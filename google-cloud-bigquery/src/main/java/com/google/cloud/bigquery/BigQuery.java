...
public interface BigQuery extends Service<BigQueryOptions> {
  ...
  /**
   * Runs a synchronous query associated with the request using a job id generated by the server.
   *
   * <p>Does not create a job first.  Instead a single HTTP request is made with the query; if it times out, it is up
   * to the caller to wait for the results using the returned job id.  If the query succeeds before the timeout,
   * then {@link SyncTableResult#getTableResult()} ()} will contain the results, otherwise it will be null and
   * {@link SyncTableResult#getJobId()} may be used to fetch the Job and wait for its results.  This method is not
   * as convenient as {@link BigQuery#query(QueryJobConfiguration, JobOption...)} but it offers a lower-latency
   * response when the expectation is that the query will be cached or small enough to complete quickly.
   *
   * <p>This method cannot be used in conjuction with {@link QueryJobConfiguration#dryRun()}
   * queries. Since dry-run queries are not actually executed, there's no way to retrieve results.
   *
   * <p>Only {@link QueryResultsOption#maxWaitTime(long)} and {@link QueryResultsOption#pageSize(long)} are valid
   * QueryResultsOption values.
   *
   * @throws BigQueryException upon failure
   * @throws InterruptedException if the current thread gets interrupted while waiting for the query
   *     to complete
   * @throws JobException if the job completes unsuccessfully
   */
  SyncTableResult syncQuery(QueryJobConfiguration configuration, QueryResultsOption... options)
      throws InterruptedException, JobException;
